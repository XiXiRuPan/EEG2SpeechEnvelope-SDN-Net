...
def prepare_dataset(train_datapath, keyword,batch_size,suffix=".npy"):
    train_data = getData(train_datapath,keyword,suffix=suffix)

    train_dataset = MyDataset(train_data,suffix=suffix)
    # Create a dataloader
...
def initial_models(params):
    ...
    subject_model = nn.DataParallel(subject_model.cuda(), device_ids=gpus)
    subject_model.load_state_dict(torch.load('result/SATDNN/middle_10.pth'))
    #optimizer = torch.optim.Adam(model.parameters(),lr=params['init_lr'])

    club_model = CLUB(3840,3840,512)
    club_model = nn.DataParallel(club_model.cuda(), device_ids=gpus)

    optimizer =  torch.optim.Adam([{'params': model.parameters(), 'lr': params['init_lr']}])
    optimizer_mi =  torch.optim.Adam([{'params': club_model.parameters(), 'lr': params['init_lr']}])
    ...
    metric = PearsonLoss()
    return model, subject_model,club_model,  optimizer, optimizer_mi,criterion,metric
'''
    model.train()
...
def train_model(params,train_dataloader,test_dataloader):
    #
    model, subject_model, club_model, optimizer, optimizer_mi, criterion,metric = initial_models(params)
    ...
    for epoch in range(num_epochs):
        ...
        adjust_learning_rate(params,optimizer_mi, epoch)
        ...
        for eeg, envelop, subject in train_dataloader:
            ...
            eeg_subject = eeg.transpose(1,2).cuda(device=torch.device('cuda:0'))
            ...
            subject_embed = subject_model(eeg_subject) 
            mi_loss = club_model(subject_embed, y_pred.squeeze(2)).mean() 
             
            mi_loss_learning = club_model.module.learning_loss(subject_embed, y_pred.squeeze(2))
           
            #loss = (loss_cc + peloss)/2
            error = -(metric(envelop, y_pred)) + 1
            #MI ESTIMATOR Training
            if epoch+1 % 10 ==0:
               for param in club_model.parameters():
                   param.requires_grad = True
               optimizer_mi.zero_grad()
               mi_loss_learning.backward()
               optimizer_mi.step()
            
            else:
               loss = loss + mi_loss
               # Envelope reconstrcution model training
               for param in club_model.parameters():
...
if __name__ =='__main__':
   ...
   config_file = 'params/subject_disentangle.yaml' 
