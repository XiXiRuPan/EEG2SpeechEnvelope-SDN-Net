...
class MLA_Codec(nn.Module):
    """
    UNet - Basic Implementation
    Paper : https://arxiv.org/abs/1505.04597
    """
    def __init__(self, in_ch=64, out_ch=1):
        super(MLA_Codec, self).__init__()
        pdb.set_trace()
        n1 = 64
        # [64,128,256,512,1024]]
...
class R2MLA_Codec(nn.Module):
    """
    R2U-Unet implementation
    Paper: https://arxiv.org/abs/1802.06955
    """
    def __init__(self, img_ch=3, output_ch=1, t=2):
        super(R2MLA_Codec, self).__init__()
...
class Attention_block(nn.Module):
    ...
    def forward(self, g, x):
        g1 = self.W_g(g)
        x1 = self.W_x(x)
        if x1.size()[-1] != g1.size()[-1]:
           g1 = F.pad(g1, (0, 1, 0, 0), mode='constant', value=0)
           #g1 =  F.pad(g1, pad)
        psi = self.relu(g1 + x1)
        psi = self.psi(psi)
        out = x * psi
...
class AttMLA_Codec(nn.Module):
    """
    Attention Unet implementation
    Paper: https://arxiv.org/abs/1804.03999
    """
    def __init__(self, img_ch=3, output_ch=1):
        super(AttMLA_Codec, self).__init__()
        ...
        self.Up_conv2 = conv_block(filters[1], filters[0])
        self.Conv = nn.Conv1d(filters[0], output_ch, kernel_size=1, stride=1, padding=0)
...
class R2AttMLA_Codec(nn.Module):
    """
    Residual Recuurent Block with attention Unet
    Implementation : https://github.com/LeeJunHyun/Image_Segmentation
    """
    def __init__(self, in_ch=64, out_ch=1, t=2):
        super(R2AttMLA_Codec, self).__init__()
        n1 = 64
        filters = [n1, n1 * 2, n1 * 4, n1 * 8, n1 * 16]
    ...
    def forward(self, x):
        ...
        if e4.size()[-1] != d5.size()[-1]:
           d5 = F.pad(d5, (0, 1, 0, 0), mode='constant', value=0)
        d5 = torch.cat((e4, d5), dim=1)
        d5 = self.Up_RRCNN5(d5)

        d4 = self.Up4(d5)
        e3 = self.Att4(g=d4, x=e3)
        if e3.size()[-1] != d4.size()[-1]:
           d4 = F.pad(d4, (0, 1, 0, 0), mode='constant', value=0)
        d4 = torch.cat((e3, d4), dim=1)
        d4 = self.Up_RRCNN4(d4)
        ...
        if e2.size()[-1] != d3.size()[-1]:
           d3 = F.pad(d3, (0, 1, 0, 0), mode='constant', value=0)
        ...
        if e1.size()[-1] != d2.size()[-1]:
           d2 = F.pad(d2, (0, 1, 0, 0), mode='constant', value=0)
...
ratt_umodel = R2AttMLA_Codec()
print(ratt_umodel)
ratt_umodel(torch.Tensor(16,64,640))

flots, params = profile(ratt_umodel, inputs=(torch.Tensor(16,64,640),))
